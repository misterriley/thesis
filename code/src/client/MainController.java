...
public class MainController
{
	...
	private static final String	QUESTION_1			=
		"In a given week, how likely are you to play one or more video games?";
	private static final String	QUESTION_2			=
		"In a given week, how likely are you to play AT LEAST FIVE HOURS of video games?";
	...
	private static final boolean[] NEUROTICISM_Q_IS_REVERSED = {false, true, false, false, true, false, true, false};
	...
	public static void runTasks()
	{
		do
		{
			if (Constants.TEST_TYPE == null || Constants.TEST_TYPE == m_model.currentTaskType())
			{
				initializeTask();
				runTask();
			}
	...
	public static void submitResults(final GameRecord p_record, final JFrame p_frame)
	{
		final CloseableHttpClient httpclient = HttpClients.createDefault();
		final HttpPost httppost = new HttpPost(Constants.POST_URL + Constants.POST_FILE);
	...
	{
		final LikertScaleQuestion lsq =
			new LikertScaleQuestion(m_model, QUESTION_1, LikertScaleQuestion.ScaleType.LIKELY);
		...
		m_model.getTaskRecord().setGameUseType(GameUseType.getType(playsFiveHours, playsAtAll));
	}
	...
	private static void doNeuroticismInventory()
	{
		...
		final LikertScaleQuestion lsq = new LikertScaleQuestion(m_model, "", LikertScaleQuestion.ScaleType.AGREE);
		...
		final double average = runningScore / (double) NEUROTICISM_QUESTIONS.length;
	...
	{
		new AgreeSaveDisagreeTextView(m_model, Constants.INFORMED_CONSENT_FILE).showText(true, false);
	}
	...
	private static int makePost(
		final CloseableHttpClient httpclient,
		final HttpPost httppost,
		final GameRecord p_record)
		throws UnsupportedEncodingException,
		IOException,
		ClientProtocolException
	{
		int statusCode;
		httppost.setEntity(new UrlEncodedFormEntity(p_record.getData()));
	...
	{
		new SaveContinueTextView(m_model, Constants.DEBRIEFING_FILE).showText(true, false);
	}

	private static void showDescription()
	{
		new DefaultTextView(m_model, Constants.DESCRIPTION_FILE, "Continue").showText(true, false);
	}
	...
	private static void showResult(final int statusCode, final JFrame p_frame)
	{
		...
		{
			JOptionPane.showMessageDialog(p_frame, "Data successfully transmitted.");
		}
		else
		{
			new DefaultTextView(m_model, Constants.TRANSMISSION_FAIL_FILE, "OK").showText(true, false);
		}
	}
}